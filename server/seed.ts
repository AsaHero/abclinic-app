...
import { serviceCategories, allServices } from '../src/types/serviceData.js';
...
const existingCategories = (db.prepare('SELECT COUNT(*) as count FROM categories').get() as { count: number }).count;
const existingServices = (db.prepare('SELECT COUNT(*) as count FROM services').get() as { count: number }).count;
...
if (existingCategories > 0 || existingServices > 0) {
  ...
  console.log('To re-seed, delete server/services.db and run again.');
  process.exit(0);
}
...
const seedAll = db.transaction(() => {
  // Seed categories (skip 'all' pseudo-category)
  serviceCategories
