...
import { getServices, getCategories, deleteService } from '@/api/services';
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="bg-[#1a1e25] border border-white/8 rounded-2xl overflow-hidden">
          ...
          {filtered.length === 0 ? (
            <div className="px-5 py-10 text-center text-gray-500 text-sm">No services found</div>
          ) : (
            filtered.map((svc, i) => (
              <div
                key={svc.id}
              ...
              >
                ...
                <div className="flex items-center gap-1.5">
                  {svc.popular && (
                    <span title="Popular" className="p-1 bg-blue-500/15 rounded-lg">
                      <Star size={13} className="text-blue-400" />
                    </span>
                  )}
                  {svc.isSpecialOffer && (
                    <span title="Special offer" className="p-1 bg-pink-500/15 rounded-lg">
